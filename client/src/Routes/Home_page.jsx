...
export default function HomePage() {
    ...
    () =>
      scheduleData.courses.map((course) => (
        <Link key={course.id} to={`/courses/${course.id}`}>
          <div className="bg-white p-4 rounded-lg shadow-md hover:shadow-lg transition-all h-40 ">
            <h2 className="text-xl font-semibold text-gray-800">
              {course.code}
            </h2>
            <p className="text-gray-600 mt-2">{course.name}</p>
            <p className="text-gray-600 mt-2 "><span>عدد الساعات : </span>{course.creditHours}</p>
          </div>
        </Link>
      )),
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-TAF-200 via-white to-TAF-200 flex justify-center items-center p-6">
      <div className="w-full max-w-screen-xl bg-gray-50 shadow-inner shadow-gray-300 rounded-lg p-6">
        {scheduleData.name && (<><h1 className="text-3xl font-bold text-gray-800 mb-6">{scheduleData.name}</h1> <hr /></>)}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 max-h-max min-h-0 scrollbar-thumb-blue-500 my-16">
